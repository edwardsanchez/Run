...
- Simulator apps are installed and launched with `simctl`.
- Apps on physical devices are installed and launched with `devicectl`.
...
- macOS 27 or later.
- A full Xcode installation. Run uses `DEVELOPER_DIR` when it is set, otherwise it looks for Xcode in `/Applications`.
